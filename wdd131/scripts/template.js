...
let recipeNarrow = document.querySelector('#recipes');
let recipeWide = document.querySelector('#recipes-wide');
...
function search() {
    ...
    let filteredSearch = recipes.filter(function(recipe){
        return ( 
    ...
    let sortedRecipes = filteredSearch.sort(compareRecipes);

    recipeNarrow.innerHTML = '';
    recipeWide.innerHTML = '';
...
function displayRecipe(recipe) {
    let html = recipeHTML(recipe);
    let htmlWide = recipeHTMLWide(recipe);
    console.log(html)
    // console.log(htmlWide)
    
    recipeWide.innerHTML += htmlWide
    recipeNarrow.innerHTML += html
}

function tagTemplate(tags) {
    let taghtml = '';
    tags.forEach(function(tag) {taghtml += ` <button>${tag}</button> `});
    return taghtml
}
...
>`
    for (let i = 1; i <= 5; i++) {
      if (i <= rating) {
        ratehtml += `<span aria-hidden="true" class="icon-boot">⭐</span>`
      } else {
        ratehtml += `<span aria-hidden="true" class="icon-empty">☆</span>`
      }			
    }
    ratehtml += `</span>`;
    return ratehtml
...
<img src="${recipe.image}" alt="no image alts provided">

<span class="label">${tagTemplate(recipe.tags)}</span>
...
function recipeHTMLWide(recipe) {
    return `<div class=recipe-wide>
<img src="${recipe.image}" alt="no image alts provided">
<div id="recipe-right">
<span class="label">${tagTemplate(recipe.tags)}</span>
...
function init() {
    displayRecipe(recipes[randomNum]);
}
